...
Dans le cadre de la notation d’un modèle, les données de notation sont conservées dans un jeu de données dans Experience Platform. Lorsque vous avez activé l’attribution multipoint lors de la création du modèle, les données de score d’événement supplémentaires sont conservées dans un jeu de données dans Experience Platform.
